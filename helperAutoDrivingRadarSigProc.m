...
function varargout = helperAutoDrivingRadarSigProc(method,varargin)
    ...
    initialdist = 100+[0; -1; 20; 100];
    laneOffset = [0; 3; 1; -2]*laneWidth;
    initialspd = [80; 110; 100; 130]*1000/3600; % km/hr -> m/s
    egoCar = addCars(scenario,roadCenters,initialdist,laneOffset,initialspd);
    ...
    while advance(scenario)
        ...
        paths = phased.internal.twoRayPaths(radarParams.OriginPosition,...
            reshape([tgtPoses.Position],3,[]),radarParams.OriginVelocity,...
            reshape([tgtPoses.Velocity],3,[]),[1 1 1],c/fc);
    ...
    function metrics = runSimulation2ray(c,fc,rangeMax,vMax,Nsweep,...
        rngdopresp,beamformer,cfar,idxCFAR,clusterer,...
        rngestimator,dopestimator,doaest,beamscan,tracker,radar,sig)
    ...
    Nft = radar.Waveform.SweepTime*radar.Waveform.SampleRate;
    Ne = radar.ReceiveAntenna.Sensor.NumElements;
    Nr = rngdopresp.RangeFFTLength;
    Nd = rngdopresp.DopplerFFTLength;
    
    rxArray = radar.ReceiveAntenna.Sensor;
    ...
    trajectory(car, waypoints, initialspd);
    end
